...
# does not work.  So... just install it in a separate script and then the real script can use it just fine.
# -Jeremy B April 2022

# if a user doesn't have write access to their R install location, typical on Windows, we need a fallback.  We
# don't really want to mess with their actual R stuff, though, so we'll place the package needed for this
# extension inside the NetLogo extension's user directory.  That'll keep it separated and not cause trouble with anything
